...
import { CombatStatus } from "../ui/CombatStatusHelpers";
...
export function getCombatStatus(
  ...
  const engagement = world.getComponent<EngagementComponent>(
    entityId,
    "engagement"
  );
  const overwatch = world.getComponent<OverwatchComponent>(
    entityId,
    "overwatch"
  );
  ...
  if (queue?.commands.length) {
    const attackCmd = queue.commands.find((c) => c.type === "attack") as
      | AttackCommand
      | undefined;
    ...
    if (attackCmd) {
      ...
      if (targetPos) {
        ...
        if (hasMove) {
          ...
          const currentDist = MovementSystem.calculateDistance(
            pos.x,
            pos.y,
            targetPos.x,
            targetPos.y
